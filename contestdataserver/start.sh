...
sed -i "s/user=\"domjudge_user\"/user=\"${DOMSERVER_USER}\"/" /opt/wlp/usr/servers/cds/config/cdsConfig.xml
sed -i "s/password=\"domjudge_password\"/password=\"${DOMSERVER_PASSWORD}\"/" /opt/wlp/usr/servers/cds/config/cdsConfig.xml
sed -i "s,url=\"contest_url\",url=\"${DOMSERVER_URL}\"," /opt/wlp/usr/servers/cds/config/cdsConfig.xml
